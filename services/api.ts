...
const mapPackage = (row: any): Package => ({
  ...
  priceCustom: row.price_custom || row.price_daily, // Support both new and old field names
  ...
  auth: {
    ...
    updateProfile: async (userId: string, updates: { name?: string; avatarUrl?: string; email?: string; password?: string; role?: string }): Promise<void> => {
      ...
      if (Object.keys(profileUpdates).length > 0) {
        console.log('[updateProfile] Sending update:', JSON.stringify(profileUpdates));
        ...
        console.log('[updateProfile] Response:', { error, data, status });
        ...
        if (!data || data.length === 0) {
          console.warn('[updateProfile] No rows updated - possible RLS issue');
        }
      }
  ...
  admin: {
    ...
    updatePackage: async (data: Partial<Package> & { id: string }): Promise<void> => {
      const updateData: any = {
        ...
        price_custom: (data as any).priceCustom,
    ...
    createPackage: async (data: Package): Promise<void> => {
      ...
      const insertData = {
        ...
        price_custom: (data as any).priceCustom || 0,
