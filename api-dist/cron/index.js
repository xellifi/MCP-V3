...
async function handler5(req, res) {
  ...
  if (execute === "true" && step) {
    const executeHandler = await Promise.resolve().then(() => (init_execute_step(), execute_step_exports));
    return executeHandler.default(req, res);
  }
  console.log(`[Cron] Running job: ${job}`);
  switch (job) {
    case "form-followup": {
      const formFollowupHandler = await Promise.resolve().then(() => (init_form_followup(), form_followup_exports));
      return formFollowupHandler.default(req, res);
    }
    case "scheduler": {
      const schedulerHandler = await Promise.resolve().then(() => (init_scheduler(), scheduler_exports));
      return schedulerHandler.default(req, res);
    }
    case "subscription-expiry": {
      const subscriptionHandler = await Promise.resolve().then(() => (init_subscription(), subscription_exports));
