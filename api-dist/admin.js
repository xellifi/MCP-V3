...
async function handleDeleteUser(req, res) {
  try {
    ...
    const { error: subError } = await supabaseAdmin.from("user_subscriptions").delete().eq("user_id", userId);
    if (subError) {
      console.error("[Delete User] Subscription deletion error:", subError);
    } else {
      console.log("[Delete User] \u2713 Deleted from user_subscriptions");
    }
    const { error: wsError } = await supabaseAdmin.from("workspaces").delete().eq("owner_id", userId);
    if (wsError) {
...
async function handleVerifyUser(req, res) {
  try {
    ...
    const { error: profileError } = await supabaseAdmin.from("profiles").update({ email_verified: verified }).eq("id", userId);
    if (profileError) {
