...
async function handleInvoiceView(req, res) {
  const { id } = req.query;
  ...
  try {
    ...
    if (!invoiceData) {
      ...
      const { data: submission, error: submissionError } = await supabase.from("form_submissions").select("*, forms(*)").eq("id", id).single();
...
async function handleTrackView(req, res) {
  ...
  try {
    ...
    if (!trackingData) {
      ...
      const { data: submission, error: fetchError } = await supabase.from("form_submissions").select("*, forms(*)").eq("id", id).single();
...
function getCurrencySymbol(currency) {
  const symbols = { PHP: "\u20B1", USD: "$", EUR: "\u20AC", GBP: "\xA3", JPY: "\xA5" };
...
function transformSubmissionToInvoice(submission) {
  const data = submission.data || {};
  const form = submission.forms || {};
  const currencySymbols = {
    PHP: "\u20B1",
  ...
  if (data.cart && Array.isArray(data.cart) && data.cart.length > 0) {
    cartItems = data.cart;
    subtotal = data.cart.reduce((sum, item) => {
      const itemPrice = parseFloat(item.productPrice) || 0;
      const itemQty = parseInt(item.quantity) || 1;
      return sum + itemPrice * itemQty;
    }, 0);
  ...
  const customerName = data.name || data.full_name || data.customer_name || data.buyer_name || data["Full Name"] || data["Name"] || submission.subscriber_name || "Customer";
  return {
...
function getStatusBadge(status) {
  const statuses = {
    pending: { icon: "\u{1F4E6}", label: "Order Placed" },
    confirmed: { icon: "\u2705", label: "Confirmed" },
    processing: { icon: "\u2705", label: "Confirmed" },
    shipped: { icon: "\u{1F69A}", label: "Shipped" },
    delivered: { icon: "\u{1F389}", label: "Delivered" },
    ...
    <div class="container">
        ...
        <div id="invoiceCard" class="invoice-card">
                ...
                <div class="header-content">
                    <div class="company-info">
                        <div class="company-logo">
                            ${data.logo ? `<img src="${data.logo}" alt="">` : "\u{1F9FE}"}
                        </div>
                        <div>
            ...
            <div class="section">
                ...
                ${hasCartItems ? data.cartItems.map((item, index) => {
    ...
    return `
                        <div class="product-row" style="margin-bottom: 8px;">
                            ...
                            <div class="product-price">
                                ...
                                ${(item.quantity || 1) > 1 ? `<div class="product-calc">\xD7 ${item.quantity} = ${data.currencySymbol}${itemTotal.toLocaleString()}</div>` : ""}
                            </div>
                        </div>`;
  }).join("") : `<div class="product-row">
                        <div>
                        ...
                        <div class="product-price">
                            ...
                            ${data.quantity > 1 ? `<div class="product-calc">\xD7 ${data.quantity} = ${data.currencySymbol}${calculatedSubtotal.toLocaleString()}</div>` : ""}
                        </div>
                    </div>`}
            </div>
            ...
            <div class="payment-section">
                <div class="payment-info">
                    <span class="payment-icon">${data.paymentMethod.includes("Cash") ? "\u{1F4B5}" : "\u{1F4F1}"}</span>
                    <div>
...
<body>
    <div class="container">
        <div class="header">
            <div class="company-row">
                <div class="company-logo">
                    ${data.logo ? `<img src="${data.logo}" alt="">` : "\u{1F4E6}"}
                </div>
                <div>
                    <div class="company-name">${escapeHtml(data.company)}</div>
    ...
    <script>
        ...
        const steps = [
            { id: 'pending', label: 'Order Placed', icon: '\u{1F4E6}', desc: 'We received your order' },
            { id: 'processing', label: 'Confirmed', icon: '\u2705', desc: 'Order has been confirmed' },
            { id: 'shipped', label: 'Shipped', icon: '\u{1F69A}', desc: 'Package is on the way' },
            { id: 'delivered', label: 'Delivered', icon: '\u{1F389}', desc: 'Package delivered successfully' }
        ];
