...
import { Plus, Zap, Sparkles, Table, Globe, Save, Play, Settings, Wrench, X, Eye, Database, BrainCircuit } from 'lucide-react';
...
const ScheduledPostsContent: React.FC<ScheduledPostsProps> = ({ workspace }) => {
  ...
  const addNode = (type: string, position?: { x: number; y: number }) => {
    ...
    const newNode: Node = {
      ...
      // Ensure circular nodes have transparent background to avoid square box
    ...
    <div className="h-[calc(100vh-100px)] w-full -m-6 relative bg-slate-950" onDragOver={onDragOver} onDrop={onDrop}>
      ...
      <div className={`absolute left-6 top-20 md:top-24 z-10 flex flex-col gap-3 transition-opacity duration-300 ${!isToolsOpen ? 'opacity-0 pointer-events-none md:opacity-100 md:pointer-events-auto' : 'opacity-100'}`}>
        ...
        <div className={`glass-panel p-3 rounded-2xl border border-white/10 shadow-2xl space-y-3 backdrop-blur-xl transition-all duration-300 origin-top-left ${isToolsOpen ? 'scale-100' : 'scale-95 pointer-events-none md:scale-100 md:pointer-events-auto'}`}>
          ...
          <div
            onDragStart={(event) => onDragStart(event, 'visualPreview')}
            draggable
            onClick={() => addNode('visualPreview')}
            className="w-12 h-12 bg-gradient-to-br from-pink-500 to-rose-600 rounded-xl flex items-center justify-center text-white shadow-lg hover:scale-110 transition-transform cursor-grab active:cursor-grabbing"
            title="Add Preview"
          >
          ...
          <div
            onDragStart={(event) => onDragStart(event, 'visualModel')}
            draggable
            onClick={() => addNode('visualModel')}
            className="w-12 h-12 bg-gradient-to-br from-violet-500 to-purple-600 rounded-xl flex items-center justify-center text-white shadow-lg hover:scale-110 transition-transform cursor-grab active:cursor-grabbing"
          ...
          <div
            onDragStart={(event) => onDragStart(event, 'visualMemory')}
            draggable
            onClick={() => addNode('visualMemory')}
            className="w-12 h-12 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-xl flex items-center justify-center text-white shadow-lg hover:scale-110 transition-transform cursor-grab active:cursor-grabbing"
            title="Add Memory"
          >
          ...
          <div
            onDragStart={(event) => onDragStart(event, 'visualTool')}
            draggable
            onClick={() => addNode('visualTool')}
            className="w-12 h-12 bg-gradient-to-br from-indigo-500 to-blue-600 rounded-xl flex items-center justify-center text-white shadow-lg hover:scale-110 transition-transform cursor-grab active:cursor-grabbing"
            title="Add Tool"
          >
