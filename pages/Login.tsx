    ...
    <div className="h-screen py-[10px] bg-slate-50 dark:bg-slate-950 flex flex-col items-center justify-center relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10 bg-white/80 dark:bg-white/5 backdrop-blur-xl rounded-3xl overflow-hidden animate-fade-in-up border border-slate-200 dark:border-white/10 shadow-2xl flex flex-col max-h-full">
        <div className="p-8 pb-4 text-center shrink-0">
          <Link to="/" className="inline-block hover:scale-105 transition-transform duration-200 mb-6">
            <div className="flex items-center justify-center gap-3">
              ...
              <div className="flex flex-col -space-y-1 text-left">
                ...
                <span className="text-[0.6rem] font-bold tracking-[0.2em] text-indigo-500 dark:text-indigo-400 uppercase w-full flex justify-between">
                  <span>A</span><span>U</span><span>T</span><span>O</span><span>M</span><span>A</span><span>T</span><span>I</span><span>N</span>
          ...
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight mb-1">Welcome back</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400">Sign in to your account</p>
        </div>

        <form onSubmit={handleSubmit} className="p-6 space-y-4 flex-1 overflow-y-auto no-scrollbar">
          <div className="space-y-3">
            <div>
              ...
              <div className="relative group">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  <Mail className="h-4 w-4 text-slate-400 dark:text-slate-500 group-focus-within:text-indigo-500 dark:group-focus-within:text-indigo-400 transition-colors" />
                </div>
                <input
                  type="email"
                  value={email}
                  onChange={e => setEmail(e.target.value)}
                  className="w-full bg-white dark:bg-white/5 border border-slate-200 dark:border-slate-700/50 rounded-lg pl-10 pr-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 outline-none transition-all text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-600"
            ...
            <div>
              <div className="flex justify-between items-center mb-1.5">
                ...
                <Link to="/forgot-password" className="text-xs font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 dark:hover:text-indigo-300 hover:underline transition-colors">Forgot password?</Link>
              </div>
              <div className="relative group">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  <Lock className="h-4 w-4 text-slate-400 dark:text-slate-500 group-focus-within:text-indigo-500 dark:group-focus-within:text-indigo-400 transition-colors" />
                </div>
                <input
                  type="password"
                  value={password}
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              ...
              className="flex items-center justify-center gap-2 py-2.5 px-4 bg-[#1877F2] hover:bg-[#1565D8] text-white font-medium rounded-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed text-sm"
            >
              {fbLoading ? (
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 py-2.5 px-4 bg-white hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-900 dark:text-white font-medium rounded-lg transition-all border border-slate-300 dark:border-slate-600 text-sm"
            >
