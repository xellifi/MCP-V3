...
import { Receipt, ChevronDown, ChevronUp, Package, Truck, CheckCircle, Clock, Download, FileImage, Loader, XCircle } from 'lucide-react';
        ...
        <div className="relative group">
            ...
            >
                ...
                {isExecuting && (
                    <>
                        <div className="absolute w-3 h-3 rounded-full bg-blue-400 shadow-[0_0_15px_#3b82f6,0_0_30px_#3b82f6] z-[100]"
                            style={{ top: '-6px', animation: 'orbTopInvoice 2s linear infinite' }} />
                        <div className="absolute w-3 h-3 rounded-full bg-blue-400 shadow-[0_0_15px_#3b82f6,0_0_30px_#3b82f6] z-[100]"
                            style={{ bottom: '-6px', animation: 'orbBottomInvoice 2s linear infinite' }} />
                        <style>{`
                            @keyframes orbTopInvoice { 0% { left: -6px; } 100% { left: calc(100% - 6px); } }
                            @keyframes orbBottomInvoice { 0% { left: -6px; } 100% { left: calc(100% - 6px); } }
                        `}</style>
                    </>
                )}
                ...
                <div className="flex items-center gap-2">
                    <div className={`p-2 rounded-lg backdrop-blur-sm flex-shrink-0 ${isError ? 'bg-red-500' : isExecuting ? 'bg-blue-500' : isCompleted ? 'bg-emerald-500' : 'bg-purple-500/20'}`}>
                        {isError ? (
