...
const AINodeForm: React.FC<AINodeFormProps> = ({
                    ...
                    >
                        ...
                        >
                            ...
                            {previewDevice !== 'desktop' && (
                                <div className="h-5 flex-shrink-0 flex items-center justify-between px-3 text-[9px] text-slate-900 font-medium">
                                    <span>9:41</span>
                                    <span>⚡ 100%</span>
                                </div>
                            )}
                            ...
                            <div className={`h-12 flex-shrink-0 flex items-center px-4 gap-3 ${previewDevice === 'desktop'
                                ...
                                <div>
                                    <p className={`text-sm font-semibold ${previewDevice === 'desktop' && isDark ? 'text-white' : 'text-slate-900'}`}>{nodeLabel || 'AI Agent'}</p>
                                    <p className={`text-[10px] ${previewDevice === 'desktop' && isDark ? 'text-slate-400' : 'text-slate-500'}`}>Usually replies instantly</p>
                                </div>
                            </div>
                            ...
                            <div className={`h-12 flex-shrink-0 flex items-center px-3 gap-2 ${previewDevice === 'desktop'
                                ...
                                <input
                                    ...
                                    className={`flex-1 text-xs bg-transparent outline-none ${previewDevice === 'desktop' && isDark ? 'text-white placeholder-slate-500' : 'text-slate-900 placeholder-slate-400'}`}
                                />
    ...
    const WIZARD_STEPS = [
        { id: 'config', title: 'Setup', icon: Bot },
    ...
    const previewSection = (
        <div className={`overflow-y-auto ${isDark ? 'bg-slate-900/50' : 'bg-slate-100'} ${isDesktop ? 'p-6 h-full' : 'p-4 flex-1'}`}>
            <h2 className={`text-lg font-semibold ${isDark ? 'text-white' : 'text-slate-900'} flex items-center gap-2 mb-4`}>
        ...
        return (
            <div className={`fixed inset-0 z-50 ${isDark ? 'bg-slate-950/95' : 'bg-white/95'} backdrop-blur-md overflow-hidden flex flex-col`}>
                ...
                <div className={`flex-1 overflow-hidden flex ${isDark ? '' : 'bg-slate-50'}`}>
                    ...
                    <div className={`w-1/3 border-r ${isDark ? 'border-white/10' : 'border-slate-200'} overflow-y-auto p-6 relative`}>
                        {aiConfigSection}
                    </div>
    ...
    // MOBILE: Step Wizard
    return (
        <div className={`fixed inset-0 z-50 flex flex-col ${isDark ? 'bg-slate-900' : 'bg-slate-50'}`}>
            {/* Progress Bar */}
            <div className={`p-4 border-b ${isDark ? 'border-white/10' : 'border-slate-200'} bg-inherit z-10`}>
                <div className="flex items-center justify-between mb-2">
                    ...
                    <button
                        onClick={onClose}
                        className={`p-1.5 rounded-lg ${isDark ? 'bg-white/10 text-slate-300' : 'bg-slate-200 text-slate-500'}`}
                    >
                        <X className="w-4 h-4" />
                    </button>
                </div>
                <div className={`h-1.5 rounded-full overflow-hidden ${isDark ? 'bg-slate-700' : 'bg-slate-200'}`}>
                    <div className="h-full bg-indigo-500 transition-all duration-300" style={{ width: `${((mobileStep + 1) / WIZARD_STEPS.length) * 100}%` }} />
                </div>
                <div className="flex mt-3">
                    {WIZARD_STEPS.map((step, i) => {
                        const Icon = step.icon;
                        const isActive = i === mobileStep;
                        const isCompleted = i < mobileStep;
                        return (
                            <div key={step.id} className={`flex-1 flex flex-col items-center gap-1 ${isActive ? 'text-indigo-500' : (isCompleted ? (isDark ? 'text-white' : 'text-slate-700') : (isDark ? 'text-slate-600' : 'text-slate-400'))}`}>
            ...
            <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
                {mobileStep === 0 && aiConfigSection}
                {mobileStep === 1 && advancedSection}
                {mobileStep === 2 && previewSection}
            </div>
            ...
            <div className={`flex-shrink-0 p-4 pb-8 border-t flex gap-3 ${isDark ? 'border-white/10 bg-slate-900' : 'border-slate-200 bg-white'}`}>
                ...
                {mobileStep < WIZARD_STEPS.length - 1 ? (
                    <button
                        ...
                        className="flex-1 py-3 rounded-xl bg-indigo-500 hover:bg-indigo-600 text-white flex items-center justify-center gap-2 font-bold shadow-lg shadow-indigo-500/20 transition-all"
                    >
                        Next
                    </button>
                ) : (
