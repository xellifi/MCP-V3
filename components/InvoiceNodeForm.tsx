...
const InvoiceNodeForm: React.FC<InvoiceNodeFormProps> = ({
    ...
    const inputClass = isDark
        ? "w-full bg-slate-800/60 border border-slate-600/50 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-purple-500/50 outline-none transition-all placeholder-slate-500"
        : "w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-slate-900 focus:ring-2 focus:ring-purple-500/50 outline-none transition-all placeholder-slate-400";

    const labelClass = isDark ? "block text-sm font-medium text-slate-300 mb-2" : "block text-sm font-bold text-slate-700 mb-2";
    const sectionBg = isDark ? "bg-slate-800/40 border-slate-700/50" : "bg-white border-slate-200 shadow-sm";
    const activeHeaderClass = (color: string) => isDark
        ? `bg-${color}-500/20 border-${color}-500/30`
        : `bg-${color}-50 border-${color}-200 text-${color}-700`;
    const inactiveHeaderClass = isDark
        ? 'bg-slate-800/20 border-white/5 hover:bg-white/5 text-white'
            ...
            <div className="flex flex-wrap gap-1.5">
                {PRESET_COLORS.map(color => (
                    <button
                        ...
                        className={`w-6 h-6 rounded-full border-2 transition-all hover:scale-110 ${value === color ? 'border-white scale-110 shadow-md' : 'border-transparent'
                            }`}
                ...
                <div className="relative">
                    ...
                    />
                    <div
                        className={`w-6 h-6 rounded-full border-2 border-dashed flex items-center justify-center bg-gradient-to-br from-red-500 via-green-500 to-blue-500 ${isDark ? 'border-slate-500' : 'border-slate-300'}`}
                    >
    ...
    const SectionHeader = ({
        ...
        <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border transition-all ${activeSection === id
                ? activeHeaderClass(color)
                : inactiveHeaderClass
                }`}
        >
            <div className={`p-1.5 rounded-lg ${activeSection === id ? (isDark ? `bg-${color}-500/20` : `bg-${color}-100`) : (isDark ? 'bg-slate-700/50' : 'bg-slate-100')}`}>
                <Icon className={`w-4 h-4 ${activeSection === id ? `text-${color}-500` : (isDark ? 'text-slate-400' : 'text-slate-500')}`} />
            </div>
    ...
    const Toggle = ({ value, onChange: onToggle, label }: { value: boolean; onChange: (v: boolean) => void; label: string }) => (
        <div className="flex items-center justify-between">
            ...
            <button
                ...
                className={`w-12 h-6 rounded-full transition-all ${value ? 'bg-purple-500' : (isDark ? 'bg-slate-600' : 'bg-slate-200')}`}
            >
    ...
    const previewCardJSX = useMemo(() => (
        <div className="rounded-2xl overflow-hidden shadow-2xl bg-white ring-1 ring-slate-900/5">
            {/* Gradient Header */}
    ...
    const configFormJSX = useMemo(() => (
        <div className="space-y-3">
            ...
            <div className="space-y-3">
                <SectionHeader id="company" icon={Building2} title="Company Info" color="blue" />
                {activeSection === 'company' && (
                    <div className={`space-y-4 p-4 rounded-xl border animate-fade-in ${sectionBg}`}>
                        <div>
                        ...
                        <div>
                            ...
                            {companyLogo ? (
                                ...
                                <button
                                    ...
                                    className={`w-full py-4 border-2 border-dashed rounded-xl transition-all flex items-center justify-center gap-2 ${isDark ? 'border-white/20 text-slate-400 hover:text-white hover:border-purple-500/50' : 'border-slate-300 text-slate-500 hover:text-slate-800 hover:border-purple-500/50'}`}
                                >
                                    {isUploading ? (
                                        <>
                                ...
                                {TEMPLATE_STYLES.map(style => (
                                    <button
                                        ...
                                        className={`p-3 rounded-xl border transition-all text-center ${templateStyle === style.id
                                            ...
                                            : (isDark ? 'bg-black/20 border-white/10 text-slate-400 hover:text-white hover:bg-white/5' : 'bg-white border-slate-200 text-slate-600 hover:text-slate-900 hover:bg-slate-50')
                                            }`}
                                    >
            ...
            <div className="space-y-3">
                ...
                {activeSection === 'tracking' && (
                    <div className={`space-y-4 p-4 rounded-xl border animate-fade-in ${sectionBg}`}>
                        ...
                        {showOrderTracking && (
                            <div className={`space-y-3 mt-4 p-3 rounded-xl ${isDark ? 'bg-black/30' : 'bg-slate-50'}`}>
                                <label className={labelClass}>Custom Status Labels</label>
                                {['pending', 'processing', 'shipped', 'delivered'].map(status => (
                                    <div key={status} className="flex items-center gap-2">
                                        <span className={`text-xs capitalize w-20 ${isDark ? 'text-slate-500' : 'text-slate-500'}`}>{status}:</span>
                                        <input
                                            type="text"
    ...
    ), [activeSection, companyName, companyLogo, companyEmail, companyPhone, companyAddress, isUploading, uploadError, primaryColor, templateStyle, showOrderItems, showCustomerInfo, showOrderTotal, showOrderTracking, statusLabels, enablePdfDownload, enableImageDownload, isDark]);
    ...
    return (
        ...
        >
            ...
            <div className="md:hidden mb-4">
                <div className={`flex rounded-xl overflow-hidden border ${isDark ? 'border-white/10 bg-black/20' : 'border-slate-200 bg-white'}`}>
                    <button
                        ...
                        className={`flex-1 py-3 px-4 text-sm font-semibold flex items-center justify-center gap-2 transition-all ${mobileView === 'config'
                            ? 'bg-purple-500/20 text-purple-600 dark:text-purple-400 border-b-2 border-purple-500'
                ...
                <div className="relative">
                    <div className="sticky top-0">
                        <div className="flex items-center gap-2 mb-3">
                            ...
                            <span className={`text-sm font-semibold ${isDark ? 'text-white' : 'text-slate-800'}`}>Live Preview</span>
                        </div>
