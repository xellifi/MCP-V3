    ...
    return (
        <div className="relative group">
            ...
            <div className={`w-[200px] h-20 border-2 rounded-xl shadow-lg flex items-center justify-center gap-3 relative group/node transition-all ${isDark ? 'bg-[#1e1e1e]' : 'bg-white'} ${getBorderClasses()}`}>
                ...
                <div className={`w-10 h-10 rounded-lg flex items-center justify-center shadow-md ${executionStatus === 'executing' ? 'bg-amber-500' : executionStatus === 'completed' ? 'bg-emerald-500' : 'bg-gradient-to-br from-pink-500 to-rose-600'} ${isDark ? 'shadow-pink-900/40' : 'shadow-pink-500/30'}`}>
                    {executionStatus === 'executing' ? <Loader className="w-6 h-6 text-white animate-spin" /> : executionStatus === 'completed' ? <CheckCircle className="w-6 h-6 text-white" /> : <ImagePlus className="w-6 h-6 text-white" />}
                </div>
                <div className="flex flex-col">
                    <span className={`font-bold text-sm ${isDark ? 'text-white' : 'text-slate-900'}`}>Image Generator</span>
                    <span className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                        {executionStatus === 'executing' ? 'Generating...' : executionStatus === 'completed' ? 'Done!' : data.size || 'AI Image'}
                    </span>
                </div>
            </div>
