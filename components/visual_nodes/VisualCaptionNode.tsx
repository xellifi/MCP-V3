    ...
    return (
        <div className="relative group">
            ...
            <div className={`w-[200px] h-20 border-2 rounded-xl shadow-lg flex items-center justify-center gap-3 relative group/node transition-colors ${isDark
                ? 'bg-[#1e1e1e] border-teal-500/50 hover:border-teal-500'
                : 'bg-white border-slate-300 hover:border-teal-500 hover:shadow-teal-500/20'
                }`}>
                ...
                {/* Content: Icon + Text */}
                <div className={`w-10 h-10 rounded-lg flex items-center justify-center shadow-md bg-gradient-to-br from-teal-500 to-emerald-600 ${isDark ? 'shadow-teal-900/40' : 'shadow-teal-500/30'}`}>
                    <PenTool className="w-6 h-6 text-white" />
                </div>
                <div className="flex flex-col">
                    <span className={`font-bold text-sm ${isDark ? 'text-white' : 'text-slate-900'}`}>Caption Writer</span>
                    <span className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                        {data.tone || 'AI Caption'}
                    </span>
                </div>
            </div>
